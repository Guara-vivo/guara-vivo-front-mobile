...
export function HistoryScreen({ onNavigate }: HistoryScreenProps) {
	...
	const filteredRecords = allMockRecords.filter((record) => {
		...
		if (
			activeFilters.minQuantity &&
			record.quantity < activeFilters.minQuantity
		) {
			return false
		}
		...
		if (
			activeFilters.maxQuantity &&
			record.quantity > activeFilters.maxQuantity
		) {
			return false
		}
		...
		if (activeFilters.behaviors && activeFilters.behaviors.length > 0) {
			const hasMatchingBehavior = activeFilters.behaviors.some(
				(filterBehavior) => record.behaviors.includes(filterBehavior),
			)
	...
	return (
		<div className="min-h-screen bg-[var(--color-muted)] pb-24">
			...
			<div className="mx-auto max-w-6xl px-4 pt-24 md:px-6">
				{/* Filtros e Busca */}
				<div className="bg-[var(--color-card)] rounded-2xl shadow-lg p-6 mb-6">
					<div className="flex flex-col md:flex-row gap-4">
						<div className="flex-1 relative">
							<Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--color-secondary-light)]" />
							<Input
								type="text"
						...
						<Button
							...
							className="flex items-center gap-2 px-6 py-3 uppercase text-sm tracking-wide"
						>
							...
							{Object.values(activeFilters).filter(
								...
								<span className="ml-1 w-5 h-5 bg-[var(--color-primary)] rounded-full flex items-center justify-center text-xs">
									{
										Object.values(activeFilters).filter(
											(v) =>
												v !== undefined &&
				...
				<div ref={recordsListRef} className="space-y-4">
					{currentRecords.length === 0 ? (
						<div className="bg-[var(--color-card)] rounded-2xl shadow-lg p-12 text-center">
						...
						currentRecords.map((record) => (
							<div
								key={record.id}
								className="bg-[var(--color-card)] rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow"
							>
								<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
									<div className="flex-1">
										<div className="flex items-center gap-3 mb-3">
											<div className="bg-[var(--color-primary)] text-[var(--color-primary-foreground)] px-3 py-1 rounded-lg uppercase text-sm tracking-wide">
												#{record.id.toString().padStart(3, '0')}
											</div>
											<div className="flex items-center gap-2 text-[var(--color-foreground)]">
												<Calendar className="w-4 h-4 text-[var(--color-secondary-light)]" />
												<span>
										...
										<div className="mt-3 flex flex-wrap gap-2">
											{record.behaviors.map((behavior) => (
												<span
													key={behavior}
													className="px-3 py-1 bg-[var(--color-secondary-light)]/20 text-[var(--color-secondary)] rounded-lg text-xs uppercase tracking-wide"
												>
													{behavior}
												</span>
											))}
						...
						<button
							...
							className={`px-4 py-2 rounded-lg transition-colors ${
								currentPage === 1
									...
									: 'bg-[var(--color-card)] border-2 border-[var(--color-secondary-light)] text-[var(--color-secondary)] hover:bg-[var(--color-muted)]'
							}`}
						>
							Anterior
						</button>
				...
				{filteredRecords.length > 0 && (
					<div className="mt-4 text-center text-sm text-[var(--color-muted-foreground)]">
						...
						{Math.min(indexOfLastRecord, filteredRecords.length)} de{' '}
